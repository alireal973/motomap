...
def build_feature_overlays(
    ...
    standard_nodes = selected_paths_by_mode.get("standart", [])
    curvy_nodes = selected_paths_by_mode.get("viraj_keyfi", [])
    safe_nodes = selected_paths_by_mode.get("guvenli", [])

    standard_records = path_edge_records(
        graph, standard_nodes, weight_attr=mode_weight_attr("standart")
    )
    curvy_records = path_edge_records(
        graph, curvy_nodes, weight_attr=mode_weight_attr("viraj_keyfi")
    )
    safe_records = path_edge_records(
        graph, safe_nodes, weight_attr=mode_weight_attr("guvenli")
    )

    speed_source = build_keyed_clusters(
        standard_records,
        key_fn=lambda item: (
            "sprint"
    ...
    traffic_source = build_keyed_clusters(
        standard_records,
        key_fn=lambda item: (
            "bypass"
    ...
    shield_source = build_keyed_clusters(
        [
            record for record in safe_records if not record["high_risk"] and record["viraj_tehlike"] == 0
        ] or safe_records,
        key_fn=lambda item: (
            "watch"
    ...
    shield_segments = [
        segment_entry(
            ...
            tone=(
                "watch"
                if record.get("cluster_key") == "watch" or record["high_risk"] or record["viraj_tehlike"] > 0
                else "safe"
            ),
        )
        for record in select_showcase_records(
    ...
    curve_source = build_signal_clusters(
        curvy_records,
    ...
    curve_segments = [
        segment_entry(
            ...
            tone=(
                "hairpin" if record["hairpin"] > 0 else "fun"
                if record["viraj_fun"] > 0 else "danger"
            ),
        )
        for record in select_showcase_records(
            curve_source,
    ...
    elevation_source = build_keyed_clusters(
