"""Advanced grid calibration for MOTOMAP on full Istanbul.

This CLI sweeps candidate values for:
- MOTOMAP_SPEED_FACTOR
- MOTOMAP_SEGMENT_DELAY_S

For each candidate it runs `evaluate_with_google.py`, parses the output JSON,
computes an objective score, and prints rich summaries.
"""
...
class RichHelpFormatter(
    ...
    """Keep defaults and preserve multiline examples."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Advanced MOTOMAP calibration for Istanbul by grid-searching speed and "
            "segment-delay candidates against evaluate_with_google.py outputs."
        ),
        formatter_class=RichHelpFormatter,
        epilog=(
            "Examples:\n"
            "  python calibrate_istanbul_advanced.py --batch 30\n"
            "  python calibrate_istanbul_advanced.py --speed-factors 0.45,0.55,0.65 "
            "--segment-delays 0,1,2 --artifacts-dir routes/calibration_artifacts\n"
            "  python calibrate_istanbul_advanced.py --dry-run"
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Evaluation script to execute per candidate.",
    )
    parser.add_argument(
        "--output-json",
    ...
    parser.add_argument(
        "--artifacts-dir",
        help=(
            "Optional directory to keep per-candidate evaluation JSON outputs. "
            "If omitted, temporary files are cleaned automatically."
        ),
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of O-D samples evaluated for each candidate.",
    )
    parser.add_argument(
        "--pairs-file",
        help="Optional explicit O-D pairs file passed to evaluation script.",
    )
    parser.add_argument(
        "--place",
    ...
    parser.add_argument("--seed", type=int, default=42, help="Seed for O-D sampling.")
    parser.add_argument(
        "--min-distance-m",
        type=float,
        default=1000.0,
    ...
    parser.add_argument(
        ...
        default="google",
        help="Baseline backend forwarded to evaluation script.",
    )
    parser.add_argument(
        "--valhalla-url",
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Penalty mismatch value when ratio metrics are missing.",
    )
    parser.add_argument(
        "--failed-run-penalty",
        type=float,
        default=1000.0,
        help="Objective assigned when subprocess run itself fails.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=12,
        help="How many ranked candidates to show in rich summary table.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the candidate grid and stop without running evaluations.",
    )
...
def extract_aggregate_metrics(summary: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(batch, dict):
        raise ValueError("Evaluation summary JSON is missing the 'batch' section.")

    cases = [case for case in (batch.get("cases") or []) if isinstance(case, dict)]
    failed_checks_raw = batch.get("failed_checks")
    failed_checks = failed_checks_raw if isinstance(failed_checks_raw, dict) else {}
...
def build_setup_panel(
    *,
    ...
    lines = [
        ...
        f"[bold]Batch per candidate:[/bold] {args.batch}",
        f"[bold]Evaluation script:[/bold] {eval_script}",
        f"[bold]Backend:[/bold] {args.backend}",
        f"[bold]Timeout:[/bold] {args.timeout_s}s",
        f"[bold]Artifacts:[/bold] {artifacts_text}",
        f"[bold]Report:[/bold] {output_json}",
    ]
    return Panel.fit("\n".join(lines), title="Calibration Setup", border_style="cyan")


def build_candidates_table(candidates: list[Candidate]) -> Table:
    table = Table(title="Candidate Grid")
    table.add_column("#", justify="right")
    table.add_column("speed_factor", justify="right")
    table.add_column("segment_delay_s", justify="right")

    for candidate in candidates:
        table.add_row(
            str(candidate.index),
            f"{candidate.speed_factor:.6g}",
            f"{candidate.segment_delay_s:.6g}",
        )
    return table


def build_results_table(results: list[dict[str, Any]], top_k: int) -> Table:
    table = Table(title=f"Top {max(1, top_k)} Candidates")
    table.add_column("rank", justify="right")
    table.add_column("idx", justify="right")
    table.add_column("speed", justify="right")
    table.add_column("delay_s", justify="right")
    table.add_column("objective", justify="right")
    table.add_column("fail_ratio", justify="right")
    table.add_column("time_mm", justify="right")
    table.add_column("dist_mm", justify="right")
    ...
    for rank, result in enumerate(ranked, start=1):
        ...
        avg_time_mismatch = to_safe_float(aggregate.get("avg_time_mismatch"))
        avg_distance_mismatch = to_safe_float(aggregate.get("avg_distance_mismatch"))
        objective = to_safe_float(result.get("objective_score"))
        duration_s = to_safe_float(result.get("duration_s"))
        ...
        table.add_row(
            str(rank),
            str(result.get("candidate_index", "-")),
            fmt_optional(to_safe_float(result.get("speed_factor")), 4),
            fmt_optional(to_safe_float(result.get("segment_delay_s")), 4),
            fmt_optional(objective, 4),
            fmt_optional(failure_ratio, 4),
            fmt_optional(avg_time_mismatch, 4),
            fmt_optional(avg_distance_mismatch, 4),
            f"[{style}]{status}[/{style}]",
            fmt_optional(duration_s, 2),
        )
...
def build_status_panel(
    *,
    ...
    status_parts = [f"{status}={count}" for status, count in sorted(counts.items())]
    success_count = sum(1 for result in results if result.get("status") == "ok")

    lines = [
        f"[bold]Candidates run:[/bold] {len(results)}",
        f"[bold]Successful:[/bold] {success_count}",
        f"[bold]Status counts:[/bold] {' | '.join(status_parts) if status_parts else '-'}",
        f"[bold]Report path:[/bold] {output_json}",
    ]
...
def main() -> None:
    ...
    with context as run_dir_ref:
        ...
        for candidate in tqdm(candidates, desc="Evaluating candidates", unit="candidate"):
            ...
            status = str(result.get("status", "unknown"))
            if status != "ok":
                error = result.get("error") or "(no error text)"
                tqdm.write(
                    f"[cand {candidate.index:03d}] {status} | sf={candidate.speed_factor:.4g} "
                    f"sd={candidate.segment_delay_s:.4g} | {error}"
                )
    ...
    report = {
        ...
        "mode": "istanbul_advanced_calibration",
        "objective": {
            "formula": (
    ...
    console.print(build_results_table(results, top_k=max(1, args.top_k)))

    if best:
        console.print(
            Panel.fit(
                "Use these env vars for follow-up runs:\n"
                f"MOTOMAP_SPEED_FACTOR={best['speed_factor']}\n"
                f"MOTOMAP_SEGMENT_DELAY_S={best['segment_delay_s']}",
